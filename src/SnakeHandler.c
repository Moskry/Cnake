...
void SnakeMove(SharedContent *content, bool *GameOver, Queue **Tail) {
  ...
  while (*GameOver != true) {
    ...
    case 'A':
      end = SnakeMoveLeft(content, Tail);
      break;
    case 'W':
      end = SnakeMoveUp(content, Tail);
      break;
    case 'S':
      end = SnakeMoveDown(content, Tail);
      break;
    case 'D':
      end = SnakeMoveRight(content, Tail);
      break;
    }
    WaitForSingleObject(content->mutex, INFINITE);
    *GameOver = end;
    ...
    if (TSC != content->Score) {
      if (content->Score <= 10)
        clock -= 10;
      else if (content->Score <= 20)
        clock -= 9;
      else if (content->Score <= 30)
        clock -= 8;
      else if (content->Score <= 40)
        clock -= 7;
      else if (content->Score <= 50)
        clock -= 6;
      else
        clock -= 1;
      TSC = content->Score;
...
bool SnakeMoveDown(SharedContent *content, Queue **Tail) {
  if (content->pos->y == content->Height - 1)
    return true;
...
bool SnakeMoveRight(SharedContent *content, Queue **Tail) {
  if (content->pos->x == content->Width * 2 - 2)
    return true;
...
bool SnakeMoveV(SharedContent *content, IntTuple newPos, Queue **Tail) {
  ...
  if (content->Map[newPos.y][newPos.x] == 'O')
    return true;
  ...
  if (content->Map[newPos.y][newPos.x] == '+') {
    ...
    // OverwriteS((IntTuple){ 7, content->Height }, (IntTuple){ 7,
    // content->Height }, score);
  }
  ...
  if (!HasTail)
    ...
    content->Map[content->pos->y][content->pos->x] = 'O';
  ReleaseMutex(content->mutex);

  if (IsApple)
    RandomNewApple(content, NULL, NULL);
